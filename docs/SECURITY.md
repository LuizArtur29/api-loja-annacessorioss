...
Em 6 de agosto de 2026, `npm audit --omit=dev` informa dois achados de severidade alta no `react-router` 7.18.2, incluindo `GHSA-qwww-vcr4-c8h2`, relacionados ao modo RSC/Server Actions. Essa é a versão mais recente publicada, e o ERP usa somente o roteamento declarativo no navegador (`BrowserRouter`, `Routes` e `Route`): não há RSC, SSR, loaders, actions ou endpoints do React Router no servidor.
